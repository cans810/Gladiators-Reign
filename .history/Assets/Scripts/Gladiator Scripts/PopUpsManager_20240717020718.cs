...
public class PopUpsManager : MonoBehaviour
{
    ...
    public void HitPopUp(float amount_GotHit, GameObject opponent)
    {
        StartCoroutine(HitPopUpCoroutine(amount_GotHit, opponent));
    }

    IEnumerator HitPopUpCoroutine(float amount_GotHit, GameObject opponent)
    {
        // Randomize the position around the opponent
        float randomDistanceX = Random.Range(-0.7f, 0.7f);
        float randomDistanceY = Random.Range(3f, 4f); // Adjust the Y range to position it above the opponent
