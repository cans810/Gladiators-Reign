...
public class CommonActions : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (GetComponent<GLAttributes>().alive){
    ...
    public void AttackMelee(){
        if (gladiatorManager.battleAI.currentEnemyChosen != null && gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive){
    ...
    public void DamageMelee(){
        if (gladiatorManager.HitBox.objectsInHitbox.Contains(gladiatorManager.battleAI.currentEnemyChosen)){
            ...
            else if (gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().dying || !gladiatorManager.battleAI.currentEnemyChosen.GetComponent<Attributes>().alive){
    ...
    IEnumerator KeepDistanceCoroutine()
    {
        ...
        GetComponent<Rigidbody2D>().velocity = direction * GetComponent<Attributes>().walk_speed;
    ...
    IEnumerator WalkToPointCoroutine(Vector2 targetPosition, float speed)
    {
        ...
        while (!hasArrivedToWalkDest(targetPosition))
        {
            ...
            float movementAmount = Mathf.Min(GetComponent<Attributes>().walk_speed * Time.deltaTime * speed, distanceToTarget);
    ...
    private IEnumerator getKilledCoroutine(string animationName){
        ...
        GetComponent<Attributes>().alive = false;
    }
