...
public class CommonActions : MonoBehaviour
{
    ...
    public void AttackMelee(){
        if (attributes.battleAI.currentEnemyChosen != null){
            ...
            Player.Instance.GetComponent<AnimationsManager>().StartAnim("RestCampfire",false);
        }
    }
    ...
    {
        animator.SetBool("TakeGuard", true);
    ...
    public void StopTakeGuard(){
        animator.SetBool("TakeGuard", false);
    ...
    {
        animator.SetBool("BeforeFight_1", true);

        // Wait until the animation ends
        yield return new WaitForSeconds(animator.GetCurrentAnimatorStateInfo(0).length);

        // Set the animation parameter to false
        animator.SetBool("BeforeFight_1", false);
    }
