...
public class GearController : MonoBehaviour
{
    ...
    void Start()
    {
        if (inventory.HelmetWorn != null){
            GameObject itemGenerated = Instantiate(AllItemsContainer.Instance.GetItem(currentSelectedItem.GetComponent<UI_Item>().itemName));
            WearHelmet(inventory.HelmetWorn);
        }
        if (inventory.ChestplateWorn != null){
            WearChestplate(inventory.ChestplateWorn);
        }
        if (inventory.ShoulderguardWorn != null){
            WearShoulderguard(inventory.ShoulderguardWorn, inventory.ShoulderguardWorn);
        }
        if (inventory.WristGuardWorn != null){
            WearWristguard(inventory.WristGuardWorn, inventory.WristGuardWorn);
        }
        if (inventory.PantsWorn != null){
            WearPants(inventory.PantsWorn);
        }
