...
public class CommonActions : MonoBehaviour
{
    ...
    IEnumerator KeepDistanceCoroutine(Transform enemyTransform)
    {
        ...
        // Calculate a random distance between 1f and 6f
        float randomDistance = UnityEngine.Random.Range(1f, 6f);

        // Calculate direction away from the enemy
        Vector2 direction = ((Vector2)transform.position - (Vector2)enemyTransform.position).normalized;

        // Calculate the target position that maintains the random distance from the enemy
        Vector2 targetPosition = (Vector2)transform.position + direction * randomDistance;
        ...
        while (Vector2.Distance(transform.position, targetPosition) > 0.01f)
        {
            ...
            Vector2 lookDirection = (enemyTransform.position - transform.position).normalized;
