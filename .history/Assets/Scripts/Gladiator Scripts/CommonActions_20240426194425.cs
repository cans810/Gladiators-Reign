...
public class CommonActions : MonoBehaviour
{
    ...
    public Vector2 ImproveTargetPos(Vector2 targetPosition)
    {
        ...
        {
            float borderYPosition = borderObject.transform.position.y;

            if (targetPosition.y >= borderYPosition - 0.4f)
            {
                // Adjust the target position to be just below the border's Y position
    ...
    public void getKilled(){
        ...
        // Create a new coroutine to wait for animation completion
        StartCoroutine(WaitForAnimationCompletion("Death_1"));
    }

    private IEnumerator WaitForAnimationCompletion(string animationName){
        // Get the Animator component (assuming it's on the same GameObject)
        Animator animator = GetComponent<Animator>();

        // Safety check: Ensure animator exists
        if (animator == null)
        {
        ...
        // Hash the animation name for efficient comparison
        int animationHash = Animator.StringToHash(animationName);

        // Wait until the current normalized time of the target animation is >= 1.0f (fully played)
        while (animator.GetCurrentAnimatorStateInfo(0).normalizedTime < 1.0f &&
            animator.GetCurrentAnimatorStateInfo(0).fullPathHash == animationHash)
        {
            yield return null; // Pause the coroutine for a frame
        }

        // Animation is complete, proceed with your next actions here
        Debug.Log("Death animation finished!");

        // Example follow-up action (replace with your desired logic)
        Destroy(gameObject); // Remove the object after animation
    }
