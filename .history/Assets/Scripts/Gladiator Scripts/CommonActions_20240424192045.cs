    ...
    IEnumerator WalkToPointMeleeAttackCoroutine()
{
    ...
    // Calculate the initial target position and improve it
    ...
    {
        // Update the target position during movement
        attributes.battleAI.targetWalkPos = ImproveTargetPos(attributes.battleAI.currentEnemyChosen.transform.position);

        // Recalculate direction towards the updated target position
        direction = (attributes.battleAI.targetWalkPos - (Vector2)transform.position).normalized;

        // Apply velocity to continue moving towards the updated target position
        GetComponent<Rigidbody2D>().velocity = direction * attributes.walk_speed;

        yield return null; // Wait for the next frame
    }

    // Stop walking animation and movement
    ...
    IEnumerator KeepDistanceCoroutine()
    {
        ...
        // Improve the target position to ensure it's within bounds
        ...
        {
            // Recalculate the direction to the target position
