...
public class GLCommonActions : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (glManager != null && glManager.battleAI != null && glManager.state.alive && glManager.battleAI.startAI){
    ...
    public void DamageMelee(){
        if (glManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)){
            ...
            && (!glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().dying && glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().alive)){
                ...
                glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().HP -= glManager.attributes.meleeDMG;
    ...
    public IEnumerator OpponentGotHitCoroutine(GameObject opponent){
        ...
        // blood splash effect
        glManager.gLEffectsManager.BloodSpashEffect(opponent.GetComponent<GladiatorManager>().GetBodyPartPos("head"));
    ...
    IEnumerator KeepDistanceCoroutine()
    {
        ...
        float randomDistance = UnityEngine.Random.Range(1.5f, 4.5f);
    ...
    public IEnumerator getStunnedCoroutine(string stunAnimName, float duration){

        glManager.battleAI.boolActionsDict["GetStunned"] = true;
        glManager.animationsManager.StartAnim(stunAnimName);
        glManager.state.is_Stunned = true;
        glManager.battleAI.PauseAI();

        yield return new WaitForSeconds(duration);

        glManager.battleAI.boolActionsDict["GetStunned"] = false;
        glManager.animationsManager.StopAnim(stunAnimName);
        glManager.state.is_Stunned = false;
        glManager.battleAI.ResumeAI();
    }

    public float CalculateDistanceBetweenPositions(Vector2 position1, Vector2 position2)
    {
        return Vector2.Distance(position1, position2);
    }

    public List<GameObject> FindNearbyAllies() {
        List<GameObject> nearbyAllies = new List<GameObject>();
        float distanceThreshold = 4.0f; // Distance threshold to consider an ally as nearby

        // Determine the tag to look for based on the current object's tag
        string targetTag = gameObject.tag.Equals("Player") ? "Player" : "Enemy";

        GameObject[] allies = GameObject.FindGameObjectsWithTag(targetTag);

        if (allies.Length == 0) {
            Debug.LogWarning("No allies found in the scene.");
            return nearbyAllies; // Return an empty list if no allies are found
        }

        Vector3 currentPosition = transform.position;

        foreach (GameObject ally in allies) {
            if (ally != gameObject && ally.GetComponent<GLState>().alive) {
                float distanceToAlly = Vector3.Distance(ally.transform.position, currentPosition);

                if (distanceToAlly <= distanceThreshold) {
    ...
    public IEnumerator GetHealedCoroutine(float healAmount, float duration) {
        glManager.state.gettingHealed = true;

        float totalHealed = 0;
        float healPerIteration = healAmount / duration; // Amount to heal per second

        while (totalHealed < healAmount) {
            glManager.attributes.HP += healPerIteration;
            totalHealed += healPerIteration;

            glManager.gLEffectsManager.NormalHealEffect(opponent.GetComponent<GladiatorManager>().GetBodyPartPos("head"));

            // Ensure we do not overheal
            if (glManager.attributes.HP > glManager.attributes.max_HP) {
                glManager.attributes.HP = glManager.attributes.max_HP;
                break;
            }

            yield return new WaitForSeconds(1); // Wait for 1 second before the next heal
        }

        glManager.state.gettingHealed = false;
    }


    public void getHealedNormal(){
