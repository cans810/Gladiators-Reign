...
public class AnimationsManager : MonoBehaviour
{
    ...
    // Method to play "Idle" animation when not in action or animating
    private void PlayIdleWhenNotAnimating()
    {
        if (!attributes.inAction && !attributes.isAnimating)
        {
            attributes.animator.SetBool("Idle", true);
        }
        else
        {
            attributes.animator.SetBool("Idle", false);
        }
    }
