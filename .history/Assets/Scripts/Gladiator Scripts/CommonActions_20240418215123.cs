    ...
    public void DamageMelee(){
        if (attributes.HitBox.objectsInHitbox.Contains(attributes.battleAI.currentEnemyChosen)){
            if (attributes.battleAI.currentEnemyChosen != null && !attributes.battleAI.currentEnemyChosen.GetComponent<BattleAI>().boolActionsDict["TakeGuard"] ){
                ...
                if (!attributes.battleAI.currentEnemyChosen.GetComponent<Attributes>().PopupsManager.GetComponent<PopUpsManager>().hitPopUpExists && attributes.battleAI.currentEnemyChosen.GetComponent <Attributes>().gotHit){
                    attributes.battleAI.currentEnemyChosen.GetComponent<Attributes>().PopupsManager.GetComponent<PopUpsManager>().HitPopUp();
                }
