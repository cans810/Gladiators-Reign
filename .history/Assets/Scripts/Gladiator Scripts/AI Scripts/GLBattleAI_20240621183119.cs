...
public class GLBattleAI : MonoBehaviour
{
    ...
    void Update()
    {
        if (GetComponent<GLState>().alive && startAI && !GetComponent<GLState>().dying)
        {
            ...
            while (actionQueue.queue.Count < 3) 
            {
                ...
                {
                    actionQueue.Enqueue("GLCommonActions", actionsList[randomAction], true, false);
                }
            }
        }
    ...
    public void StopAI(){
        // Safely stop the AI execution
        if (startAI)
        {
            startAI = false;

            // Clear the action queue if it's not null
            if (actionQueue != null)
            {
                actionQueue.ClearQueue();
            }

            // Reset current action and action key
            currentAction = null;
            currentActionKey = string.Empty;

            // Reset all boolean action flags in the dictionary
            if (boolActionsDict != null)
            {
