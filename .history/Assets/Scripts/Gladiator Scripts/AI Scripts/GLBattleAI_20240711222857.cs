...
public class GLBattleAI : MonoBehaviour
{
    ...
    void Update()
    {
        if (GetComponent<GLState>().alive && startAI && !GetComponent<GLState>().dying)
        {
            ...
            {
                var bloodreaverActions = gameObject.GetComponent<BloodreaverActions>();

                if (bloodreaverActions != null)
                {
                    string result = bloodreaverActions.BloodreaverAI();

                    while (result != "" && result != "GetKilled")
                    {
                        // Perform any necessary operations before the next iteration, if needed
                        result = bloodreaverActions.BloodreaverAI();
                    }
                }
            }
        }
