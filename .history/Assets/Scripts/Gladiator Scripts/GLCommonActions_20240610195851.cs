...
public class GLCommonActions : MonoBehaviour
{
    ...
    IEnumerator WalkToPointMeleeAttackCoroutine()
    {
        ...
        && glManager.battleAI.currentEnemyChosen.GetComponent<>().alive)
        {
            glManager.battleAI.targetWalkPos = ImproveTargetPos(glManager.battleAI.currentEnemyChosen.transform.position);
        ...
        && (!glManager.battleAI.currentEnemyChosen.GetComponent<>().dying && glManager.battleAI.currentEnemyChosen.GetComponent<>().alive)){
            AttackMelee();
        }
        else{
            StopAttackMelee();
    ...
    public void DamageMelee(){
        if (glManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)){
            ...
            else if (glManager.battleAI.currentEnemyChosen.GetComponent<>().dying || !glManager.battleAI.currentEnemyChosen.GetComponent<>().alive){
    ...
    IEnumerator OpponentGotHitCoroutine(GameObject opponent){
        ...
        opponent.GetComponent<>().gotHit = true;
        ...
        opponent.GetComponent<>().gotHit = false;
    }
    ...
    private IEnumerator getKilledCoroutine(string animationName){
        ...
        glManager.animationsManager.StartAnim("Death_" + randomDeathAnim);
