...
public class GladiatorRosterGenerator : MonoBehaviour
{
    ...
    public void GenerateRandomGladiatorRoster(int rosterSize)
    {
        ...
        for (int i = 0; i < rosterSize; i++)
        {
            ...
            ClickableObject clickable = generatedGladiator.GetComponent<ClickableObject>();
            if (clickable == null)
            {
                clickable = generatedGladiator.AddComponent<ClickableObject>();
            }

            clickable.onClick.AddListener(() => OnGladiatorClick(generatedGladiator));
    ...
    {
        // Example action when a gladiator is clicked
        Debug.Log("Gladiator clicked: " + gladiator.name);
        // Add more actions as needed, such as showing details in the UI
    }
}
