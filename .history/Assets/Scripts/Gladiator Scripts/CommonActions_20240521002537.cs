...
public class CommonActions : MonoBehaviour
{
    ...
    IEnumerator WalkToPointMeleeAttackCoroutine()
    {
        ...
        while (!hasArrivedToAttack(attributes.battleAI.targetWalkPos) && attributes.battleAI.boolActionsDict["WalkToPointMeleeAttack"] == true 
        ...
        if (attributes.battleAI.boolActionsDict["WalkToPointMeleeAttack"] == true 
        && (!attributes.battleAI.currentEnemyChosen.GetComponent<Attributes>().dying && attributes.battleAI.currentEnemyChosen.GetComponent<Attributes>().alive)){
            AttackMelee();
        }
        else{
            StopAttackMelee();
    ...
    public void DamageMelee(){
        if (attributes.HitBox.objectsInHitbox.Contains(attributes.battleAI.currentEnemyChosen)){
            ...
            else if (attributes.battleAI.currentEnemyChosen.GetComponent<Attributes>().dying || !attributes.battleAI.currentEnemyChosen.GetComponent<Attributes>().alive){
    ...
    public void WalkToPoint(Vector2 targetPosition, float speed,bool switch_value){
        StartCoroutine(WalkToPointCoroutine(targetPosition,speed, switch_value));
    }

    IEnumerator WalkToPointCoroutine(Vector2 targetPosition, float speed, bool switch_value)
    {
