    ...
    IEnumerator KeepDistanceCoroutine(Transform enemyTransform)
{
    ...
    // Generate a random distance between 1f and 6f
    float randomDistance = UnityEngine.Random.Range(1f, 6f);

    // Calculate a random direction away from the enemy
    Vector2 randomDirection = Random.insideUnitCircle.normalized;

    // Calculate the target position that is a random distance away from the enemy
