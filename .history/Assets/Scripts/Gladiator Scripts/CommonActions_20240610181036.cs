...
public class CommonActions : MonoBehaviour
{
    ...
    public void DamageMelee(){
        if (glManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)){
            ...
            else if(glManager.battleAI.currentEnemyChosen != null && glManager.battleAI.currentEnemyChosen.GetComponent<BattleAI>().boolActionsDict["TakeGuard"]){
                Debug.Log("enemy is blocking.");
            }
            else if (glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().dying || !glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive){
