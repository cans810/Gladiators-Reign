...
public class CommonActions : MonoBehaviour
{
    ...
    IEnumerator WalkToPointMeleeAttackCoroutine()
    {
        ...
        while (!hasArrivedToAttack(attributes.battleAI.targetWalkPos) && attributes.battleAI.boolActionsDict["WalkToPointMeleeAttack"])
        {
            attributes.battleAI.targetWalkPos = ImproveTargetPos(attributes.battleAI.currentEnemyChosen.transform.position);
    ...
    IEnumerator KeepDistanceCoroutine()
    {
        ...
        while (!hasArrivedToWalkDest(targetPosition) && attributes.battleAI.boolActionsDict["KeepDistance"])
        {
    ...
    private IEnumerator getKilledCoroutine(string animationName){
        attributes.battleAI.boolActionsDict["GetKilled"] = true;

        GetComponent<AnimationsManager>().StartAnim(animationName, true);
