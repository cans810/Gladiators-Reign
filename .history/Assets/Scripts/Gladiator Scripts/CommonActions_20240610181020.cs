...
public class CommonActions : MonoBehaviour
{
    ...
    public void AttackMelee(){
        if (glManager.battleAI.currentEnemyChosen != null && glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive){
    ...
    IEnumerator WalkToPointMeleeAttackCoroutine()
    {
        ...
        GetComponent<Rigidbody2D>().velocity = direction * GetComponent<GLAttributes>().walk_speed;

        while (!hasArrivedToAttack(glManager.battleAI.targetWalkPos) && glManager.battleAI.boolActionsDict["WalkToPointMeleeAttack"] == true 
        && glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive)
        {
            glManager.battleAI.targetWalkPos = ImproveTargetPos(glManager.battleAI.currentEnemyChosen.transform.position);
        ...
        && (!glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().dying && glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive)){
            AttackMelee();
        }
        else{
            StopAttackMelee();
    ...
    public void DamageMelee(){
        if (gladiatorManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)){
            ...
            && (!gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().dying && gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive)){
                int hpBefore = (int)gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().HP;
                gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().HP -= 1;
                gladiatorManager.battleAI.currentEnemyChosen.GetComponent <GLAttributes>().amount_GotHit = hpBefore - gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().HP;

                gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GladiatorManager>().PopupsManager.GetComponent<PopUpsManager>().HitPopUp(gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().amount_GotHit
            ...
            else if(gladiatorManager.battleAI.currentEnemyChosen != null && gladiatorManager.battleAI.currentEnemyChosen.GetComponent<BattleAI>().boolActionsDict["TakeGuard"]){
                Debug.Log("enemy is blocking.");
            }
            else if (gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().dying || !gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive){
    ...
    IEnumerator OpponentGotHitCoroutine(GameObject opponent){
        ...
        opponent.GetComponent<GLAttributes>().gotHit = true;
        
        if (!opponent.GetComponent<GLAttributes>().dying || opponent.GetComponent<GLAttributes>().alive){
        ...
        opponent.GetComponent<GLAttributes>().gotHit = false;
    }

    public void StopAttackMelee(){
        GetComponent<AnimationsManager>().StopAction("AttackMelee_1");
        gladiatorManager.battleAI.boolActionsDict["WalkToPointMeleeAttack"] = false;
    ...
    {
        GetComponent<AnimationsManager>().StartAction("Walk");
        gladiatorManager.battleAI.boolActionsDict["KeepDistance"] = true;

        Vector2 enemyPosition = gladiatorManager.battleAI.currentEnemyChosen.transform.position;
        ...
        while (!hasArrivedToWalkDest(targetPosition) && gladiatorManager.battleAI.boolActionsDict["KeepDistance"] == true
        && gladiatorManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().alive)
        {
    ...
    public void StopKeepDistance(){
        GetComponent<Rigidbody2D>().velocity = Vector2.zero;
        GetComponent<AnimationsManager>().StopAction("Walk");
        gladiatorManager.battleAI.boolActionsDict["KeepDistance"] = false;
    }
    ...
    IEnumerator TakeGuardCoroutine() 
    {
        ...
        gladiatorManager.battleAI.boolActionsDict["TakeGuard"] = true;

        float elapsedTime = 0f;
        float randomDuration = Random.Range(0.5f, 1f);

        while (elapsedTime < randomDuration && gladiatorManager.battleAI.boolActionsDict["TakeGuard"]) 
        {
    ...
    public void StopTakeGuard(){
        ...
        gladiatorManager.battleAI.boolActionsDict["TakeGuard"] = false;
    ...
    IEnumerator WalkToPointCoroutine(Vector2 targetPosition, float speed)
    {
        ...
        while (!hasArrivedToWalkDest(targetPosition))
        {
            ...
            float movementAmount = Mathf.Min(GetComponent<GLAttributes>().walk_speed * Time.deltaTime * speed, distanceToTarget);
    ...
    public GameObject FindNearestEnemy()
    {
        ...
        if (gameObject.tag.Equals("Player")){
            ...
            {
                if (enemy.GetComponent<GLAttributes>().alive){
    ...
    private IEnumerator getKilledCoroutine(string animationName){
        ...
        Animator animator = gladiatorManager.animator;
        ...
        if (gladiatorManager.animator == null)
        {
        ...
        while (gladiatorManager.animator.GetCurrentAnimatorStateInfo(0).normalizedTime < 1.0f &&
            gladiatorManager.animator.GetCurrentAnimatorStateInfo(0).fullPathHash == animationHash)
        {
            yield return null;
        }

        //Debug.Log("clean dead body...");
        GetComponent<GLAttributes>().alive = false;
    }
