...
public class DetachEntityParts : MonoBehaviour
{
    public void detachHead()
    {
        ...
        if (foundHead != null)
        {
            ...
            // Copy properties from the original head
            detachedRenderer.sprite = originalRenderer.sprite;
            detachedRenderer.color = originalRenderer.color;
            detachedRenderer.sortingLayerName = originalRenderer.sortingLayerName;
            detachedRenderer.sortingOrder = originalRenderer.sortingOrder;

            // Set scale and physics properties (assuming 2D)
            ...
            // Add DetachedHeadController after setting up the GameObject
            detachedHead.AddComponent<DetachedHeadController>(); 
