...
public class RaceSelectionManager : MonoBehaviour
{
    ...
    public void nextRace(){
        if (appereanceManager.currentRace < appereanceManager.racesSpriteLibraries.Count-1){
            ...
            currentRaceText.text = appereanceManager.racesDict[appereanceManager.currentRace];
            GetComponent<RegionSelectionManager>().currentRegionText.text = playerGladiator.GetComponent<AppereanceManager>().currentRegionsDict[playerGladiator.GetComponent<AppereanceManager>().currentRegionNum];
        }
    }
    ...
    public void prevRace(){
        if (appereanceManager.currentRace > 0){
            ...
            playerGladiator.GetComponent<Attributes>().raceRegion = playerGladiator.GetComponent<AppereanceManager>().currentRegionsDict[GetComponent<RegionSelectionManager>().currentRegionNum];

            currentRaceText.text = appereanceManager.racesDict[appereanceManager.currentRace];
            GetComponent<RegionSelectionManager>().currentRegionText.text = playerGladiator.GetComponent<AppereanceManager>().currentRegionsDict[GetComponent<RegionSelectionManager>().currentRegionNum];
        }
    }
}
