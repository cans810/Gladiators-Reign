...
public class PlayerEquipmentCanvas : MonoBehaviour
{
    ...
    public void unEquipSelectedItem(){
        if (currentSelectedItem.GetComponent<UI_Item>().itemName.Contains("Helmet")){
            Player.Instance.GetComponent<GearController>().UnequipGear(Player.Instance.GetComponent<GearController>().helmetSpot, null);
        }

        if (currentSelectedItem.GetComponent<UI_Item>().itemName.Contains("Chestplate")){
            Player.Instance.GetComponent<GearController>().UnequipGear(Player.Instance.GetComponent<GearController>().chestplateSpot, null);
        }

        if (currentSelectedItem.GetComponent<UI_Item>().itemName.Contains("ShoulderGuard")){
            Player.Instance.GetComponent<GearController>().UnequipGear(Player.Instance.GetComponent<GearController>().shoulderguardSpot_right, Player.Instance.GetComponent<GearController>().shoulderguardSpot_left);
        }
    }
