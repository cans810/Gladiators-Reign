    ...
    void Start()
    {
        ...
        if (DungeonBlackSmithData.SelectedPart.Equals("Helmet")){
            for (int i=0; i<1; i++){
                ...
                ui_armor.GetComponent<UI_Item>().setItemUI(AllItemsContainer.Instance.allHelmets[i].GetComponent<ItemData>().itemName, 
                AllItemsContainer.Instance.allHelmets[i].GetComponent<ItemData>().texture);
