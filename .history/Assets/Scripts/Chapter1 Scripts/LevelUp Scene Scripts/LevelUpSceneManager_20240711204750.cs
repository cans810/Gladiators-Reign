    ...
    public void acquireSelectedSkill(){
        ...
        if (glClass == GladiatorClass.Bloodreaver){
            ...
            foreach (GameObject skill in glSkillTree.transform){
                ...
                if (skill.name.Contains(selectedSkill.name) || skill.name.Equals(selectedSkill.name)){
                    LeveledUp_Gl.GetComponent<GLBattleAI>().AddSkill(LeveledUp_Gl.GetComponent<GLAttributes>().GL_Class.ToString() + "_" + selectedSkill.name);
                }
            }
        }
    }
