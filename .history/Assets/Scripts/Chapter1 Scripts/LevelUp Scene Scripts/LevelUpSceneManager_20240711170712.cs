...
public class LevelUpSceneManager : MonoBehaviour
{
    ...
    public void continueButtonClicked(){
        ...
        activateSkillTree(LeveledUp_Gl.GetComponent<GLAttributes>().GL_Class)
    }

    public void activateSkillTree(GladiatorClass glClass, GameObject skillTree){
        GameObject glSkillTree = null;
        
        if (glClass == GladiatorClass.Bloodreaver){
            glSkillTree = skillTree.transform.Find("BloodreaverSkillTree").gameObject;
        }

        foreach (Transform child in skillTree.transform)
        {
