...
public class LevelUpSceneManager : MonoBehaviour
{
    ...
    public void acquireSelectedSkill(){
        ...
        string skillName = selectedSkill.name.Replace(" ", "").Replace("'", "");
string acquiredSkill = glClass.ToString() + "_" + skillName;

        if (LeveledUp_Gl.GetComponent<BloodreaverActions>() != null){
            LeveledUp_Gl.GetComponent<BloodreaverActions>().AddSkill(acquiredSkill);
        }

        Debug.Log("acquired skill, " + acquiredSkill);
    }
