...
public class BattleController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (allEnemiesDead && !battleHasEnded){
            ...
            Player.Instance.GetComponent<AnimationsManager>().StopAllAnim();
            Player.Instance.GetComponent<AnimationsManager>().StartAnim("BeforeFight_1");
        ...
        else if (!Player.Instance.GetComponent<GLAttributes>().alive && !battleHasEnded){
            BattleEndCanvas.SetActive(true);
            SetButtons();
            battleHasEnded = true;
            ...
            {
                if (enemy != null && enemy.GetComponent<GLAttributes>().alive) 
                {
                    //enemy.gameObject.GetComponent<BattleAI>().StopAI();
    ...
    public bool areAllEnemiesDead()
    {
        foreach (GameObject enemy in enemies)
        {
            ...
            else if (enemy.GetComponent<GLAttributes>().alive)
            {
                return false;
            }
        }
