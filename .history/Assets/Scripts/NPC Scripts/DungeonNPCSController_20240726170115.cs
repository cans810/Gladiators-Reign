...
public class DungeonNPCSController : MonoBehaviour
{
    ...
    void Update()
    {
        if (SceneManager.GetActiveScene().name.Equals("DungeonScene")){
            ...
            GameObject blackSmith = transform.Find("NPCBlacksmith").gameObject;
            blackSmith.GetComponent<NPCBlacksmithController>().npcActive = false;

            GameObject multiFightArranger = findNPCInScene("NPCMultifightArranger"); transform.Find("NPCMultifightArranger").gameObject
