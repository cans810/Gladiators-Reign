...
public class BloodreaverActions : MonoBehaviour
{
    ...
    public void DamageMelee_RavagersCleaveAttack(){
        if (glManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)){
            ...
            && (!glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().dying && glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().alive)){
                float hpBefore = glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().HP;
    ...
    public void StunEnemy_TitansChargeAttack(){
        //if (glManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)){
            if (glManager.battleAI.currentEnemyChosen != null && (!glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().dying && glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().alive)){
                int hpBefore = glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().HP;
    ...
    public void Bloodreaver_BloodforgedPact(){
        ...
        if (canUse_BloodforgedPact){
            ...
            glManager.battleAI.boolActionsDict["Bloodreaver_BloodforgedPact"] = true;
            GetComponent<Rigidbody2D>().velocity = Vector2.zero;

            glManager.animationsManager.StartAction("BloodforgedPact");

            StartCoroutine(SetBloodforgedPactCooldown(30f)); // 30 seconds cooldown

            float hpSacrificedAmount = glManager.attributes.max_HP * 20/100;

            // lost health popup for bloodforgedpact
            glManager.attributes.HP -= hpSacrificedAmount; 

            List<GameObject> nearbyAllies = glManager.commonActions.FindNearbyAllies();

            float hpHealedAmount = hpSacrificedAmount/nearbyAllies.Count;
            
            foreach(GameObject ally in nearbyAllies){
                if (!ally.GetComponent<GLState>().dying && ally.GetComponent<GLState>().alive){
                    StartCoroutine(ally.GetComponent<GLCommonActions>().GetHealedCoroutine(hpHealedAmount, 4f));
                }
            }
        }
    }

    public void StopBloodforgedPact(){
        glManager.animationsManager.StopAction("BloodforgedPact");
        glManager.battleAI.boolActionsDict["Bloodreaver_BloodforgedPact"] = false;

        //attributes.battleAI.currentEnemyChosen = null;
    }

    public IEnumerator SetBloodforgedPactCooldown(float seconds){
        canUse_BloodforgedPact = false;
