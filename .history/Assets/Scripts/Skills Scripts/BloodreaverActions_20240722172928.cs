...
public class BloodreaverActions : MonoBehaviour
{
    ...
    IEnumerator WalkTo_RavagingFuryAttack_Coroutine() {
        ...
        float minDistance = 1.5f; // Minimum distance to keep from the opponent
        glManager.battleAI.targetWalkPos = commonActions.ImproveFollowingTargetPos(glManager.battleAI.currentEnemyChosen.transform.position, minDistance);

        Vector2 direction = (glManager.battleAI.targetWalkPos - (Vector2)transform.position).normalized;

        GetComponent<Rigidbody2D>().velocity = direction * GetComponent<GLAttributes>().WalkSpeed * 1.1f;

        while (glManager.battleAI.boolActionsDict["Bloodreaver_RavagingFury"] == true 
        && glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().alive) {
            glManager.battleAI.targetWalkPos = commonActions.ImproveTargetPos(glManager.battleAI.currentEnemyChosen.transform.position, minDistance);

            if (!hasArrivedTo_RavagingFuryAttack(glManager.battleAI.targetWalkPos, minDistance)) {
                direction = (glManager.battleAI.targetWalkPos - (Vector2)transform.position).normalized;
                GetComponent<Rigidbody2D>().velocity = direction * GetComponent<GLAttributes>().WalkSpeed * 1.1f;
            } else {
                GetComponent<Rigidbody2D>().velocity = Vector2.zero;
        ...
        && glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().alive) {
            ...
            if (!hasArrivedTo_RavagingFuryAttack(glManager.battleAI.targetWalkPos, minDistance)) {
                ...
                GetComponent<Rigidbody2D>().velocity = direction2 * GetComponent<GLAttributes>().WalkSpeed * 1.1f;
            } else {
                GetComponent<Rigidbody2D>().velocity = Vector2.zero;
            }
    ...
    public void DamageMelee_RavagingFuryAttack() {
        if (glManager.HitBox.objectsInHitbox.Contains(glManager.battleAI.currentEnemyChosen)) {
            ...
            && (!glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().dying && glManager.battleAI.currentEnemyChosen.GetComponent<GLState>().alive)) {
                ...
                glManager.battleAI.currentEnemyChosen.GetComponent<GladiatorManager>().PopupsManager.GetComponent<PopUpsManager>().HitPopUp(glManager.battleAI.currentEnemyChosen.GetComponent<GLAttributes>().amount_GotHit, glManager.battleAI.currentEnemyChosen);
    ...
    public bool hasArrivedTo_RavagingFuryAttack(Vector2 targetPos, float minDistance) {
        float distanceToTarget = Vector2.Distance(transform.position, targetPos);
    ...
    public Vector2 ImproveFollowingTargetPos(Vector2 enemyPos, float minDistance){
        Vector2 direction = (enemyPos - (Vector2)transform.position).normalized;
        return enemyPos - direction * minDistance;
    }
